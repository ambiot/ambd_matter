...
1. Unzip Ameba_1-10_MP_ImageTool_MacOS.zip and make it executable
    - unzip Ameba_1-10_MP_ImageTool_MacOS.zip
    - chmod +x Ameba_1-10_MP_ImageTool_MacOS


2. Check tool usage
    - sudo ./Ameba_1-10_MP_ImageTool_MacOS -help


3. Set AmebaD chip
Example:
	- sudo ./Ameba_1-10_MP_ImageTool_MacOS -set chip AmebaD
...
Now, Ameba board gets into UART_DOWNLOAD mode and is ready to receive data.
	...
	- sudo ./Ameba_1-10_MP_ImageTool_MacOS -scan device
(Automatically add the uart device to config, so you can skip step #5)

5. Add Uart Device
Example:
	- sudo ./Ameba_1-10_MP_ImageTool_MacOS -add device /dev/ttyXXX

6. Set Baudrate
Example:
	- sudo ./Ameba_1-10_MP_ImageTool_MacOS -set baudrate 1500000

7. Combine AmebaD three images
	- sudo ./Ameba_1-10_MP_ImageTool_MacOS -combine <file1> <offset1> <file2> <offset2> ...

	- sudo ./Ameba_1-10_MP_ImageTool_MacOS -combine km0_boot_all.bin 0x0000 km4_boot_all.bin 0x4000 km0_km4_image2.bin 0x6000
       (Tool will generate the Image_All.bin after combine command.)


8. Set image file path
    - sudo ./Ameba_1-10_MP_ImageTool_MacOS -set image /<Current_PATH>/Image_All.bin


9. Set image address
    - sudo ./Ameba_1-10_MP_ImageTool_MacOS -set address 0x08000000


10. Final check
Example:
	- sudo ./Ameba_1-10_MP_ImageTool_MacOS -show
...
(Notice : please set download mode on AmebaD board before download command)
	- sudo ./Ameba_1-10_MP_ImageTool_MacOS -download
